...
- **Inline single-password audit** — check one password straight from the
  command line with `--pass`, no file needed.
...
### 4. Audit a single password inline
Skip the file entirely and check one password directly in the terminal:
```bash
java -jar target/password-auditor-1.0.0.jar --pass "MyPassword123!"

# or, if you compiled without Maven:
java -cp target/classes com.passauditor.Main --pass "MyPassword123!"
```

Always put the password in double quotes so your shell does not interpret
special characters like `&` or `!`.

> **Note:** anything typed on a command line is kept in the terminal's command
> history, so `--pass` is meant for testing and demos, not for auditing your
> real passwords. For those, use a file and delete it afterwards. A production
> tool would read it hidden via `System.console().readPassword()`.
